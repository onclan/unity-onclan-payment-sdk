...
enum {
    OCSDKCBLJSONReadingMutableContainers = (1UL << 0),
    OCSDKCBLJSONReadingMutableLeaves = (1UL << 1),
    OCSDKCBLJSONReadingAllowFragments = (1UL << 2)
};
typedef NSUInteger OCSDKCBLJSONReadingOptions;

/** Identical to the corresponding NSJSON option flags, with one addition. */
enum {
    OCSDKCBLJSONWritingPrettyPrinted = (1UL << 0),
    
    OCSDKCBLJSONWritingAllowFragments = (1UL << 23)  /**< Allows input to be an NSString or NSValue. */
};
typedef NSUInteger OCSDKCBLJSONWritingOptions;


/** Useful extensions for JSON serialization/parsing. */
@interface OCSDKCBLJSON : NSJSONSerialization
...
@interface OCSDKCBLLazyArrayOfJSON : NSArray
{
